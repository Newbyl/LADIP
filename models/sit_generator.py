...
class SiT(nn.Module):
    ...
    def __init__(
        ...
        **block_kwargs # fused_attn
        ...
        # Project to ViT predictor input dim (default 768)
        self.to_vit_dim = nn.Linear(hidden_size, z_dims[0])
        from .generator import VisionTransformerPredictor
        self.vit_predictor = VisionTransformerPredictor(
            num_patches=num_patches,  # total patches (e.g., 256)
            embed_dim=z_dims[0],      # match projected dim
            predictor_embed_dim=z_dims[0],  # or set as needed
            depth=6,                 # or set as needed
            num_heads=12, # or set as needed
            mlp_ratio=4,
            qkv_bias=True, 
    ...
    def mask_patches(self, x, mask_ratio):
        ...
        x: (N, T, D)
        Returns:
            x_unmasked: (N, T*(1-mask_ratio), D)
            mask_indices: indices of masked patches
            keep_indices: indices of unmasked patches
        """
        N, T, D = x.shape
        ...
        x_unmasked = torch.gather(x, 1, keep_indices.unsqueeze(-1).expand(-1, -1, D))
        return x_unmasked, keep_indices, mask_indices
    ...
    def forward(self, x, t, y, return_logvar=False):
        ...
        for i, block in enumerate(self.blocks):
            ...
            # After encoder_depth blocks, do the masking prediction (always during training)
            if (i + 1) == self.encoder_depth:
                # Project to ViT predictor input dim
                x_proj = self.to_vit_dim(x)  # (N, T, vit_dim)
                # Mask patches
                x_unmasked, keep_indices, mask_indices = self.mask_patches(x_proj, self.mask_ratio)
                # Prepare masks for ViT predictor (each element is indices for one sample)
                masks_x = [keep_indices[i] for i in range(N)]
                masks = [mask_indices[i] for i in range(N)]
                # ViT predictor returns predictions for masked patches
                # Always run predictor to ensure all parameters get gradients
                pred_masked = self.vit_predictor(x_unmasked, masks_x, masks)  # (N, num_masked, vit_dim)
                
        x = self.final_layer(x, c)                # (N, T, patch_size ** 2 * out_channels)
        x = self.unpatchify(x)                   # (N, out_channels, H, W)

        return x, pred_masked, mask_indices
